...
CREATE UNIQUE INDEX idx_findings_open_fingerprint
    ON findings (fingerprint) WHERE status = 'OPEN';

-- Ledger rows are never deleted.
CREATE TRIGGER runs_never_deleted
BEFORE DELETE ON runs
BEGIN
    SELECT RAISE(ABORT, 'ledger rows are never deleted');
END;

CREATE TRIGGER tasks_never_deleted
BEFORE DELETE ON tasks
BEGIN
    SELECT RAISE(ABORT, 'ledger rows are never deleted');
END;

CREATE TRIGGER findings_never_deleted
...
CREATE TRIGGER findings_lifecycle_guard
